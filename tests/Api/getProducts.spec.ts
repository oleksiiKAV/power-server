...
test.describe.parallel('Get products information', () => {
  ...
  test(`Get healthy recommended products with category  - by blood type. /products/admissible/?category=flour&filter=true`, async ({ request }) => {
    const userData: UserData = singnUpValidBody.userData;
    const responseBody = await signUpAndLogin(request, userData);
    const token = responseBody.token;
    ...
    try {
      const respGet = await request.get(`${baseUrl}/products/admissible/?category=flour&filter=true`,
        {
          headers: { "Authorization": `Bearer ${token}` },
        });
      expect(respGet.status()).toBe(200);
      ...
      respGetBody.forEach(element => {        
        expect(element.groupBloodNotAllowed[userBloodType]).toBe(true)
        expect(element.category).toBe("flour")
      });
    }
    finally {
      const deleteResponse = await deleteUser(request, token);
